---
name: worktree-close-workflow
description: Workflow: worktree-close-workflow
---

# Workflow: worktree-close-workflow

> 자동 생성된 스킬입니다. 직접 수정하지 마세요.
> 소스: `~/.claude/skills/dynamic-workflow-builder/src/templates/worktree-close-workflow.yaml`
...
|------|-------|-------------|
| task-close | task-lifecycle-closer | 현재 worktree의 작업 내용을 요약하고 사용자 확인 후 worktree를 종료한다 |
...
1. **[STEP]** `task-close`
   - desc: 현재 worktree의 작업 내용을 요약하고 사용자 확인 후 worktree를 종료한다
   - agent: `task-lifecycle-closer` → Agent 도구의 subagent_type
   - details (필수 준수): `references/worktree-close.md` — 에이전트는 이 파일에 정의된 지침을 반드시 따른다
   - 산출물: `.local/$ARGUMENTS/task-close.md`
