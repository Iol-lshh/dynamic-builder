  ...
  steps:
    - {step-name}:
        ...
        details:                # optional, src/detailss/ 기준 (워크플로우 로컬)
          - {details-file-path}
        refs:                  # optional, __AGENT_HOME__/references/ 기준 (글로벌)
          - {reference-file-path}
        input:                 # optional
          - {step-name}
...
| 에이전트명 | Y | `__AGENT_HOME__/agents/` 아래의 실제 에이전트 파일명 |
...
|---|---|---|
| desc | Y | task 설명. |
| agent | N | 사용할 agent 이름 (`__AGENT_HOME__/agents/` 아래) 또는 `define.agents`의 별칭. 생략 시 오케스트레이터가 직접 수행 |
...
| refs | N | agent가 참조할 파일 목록. 베이스명 → `__AGENT_HOME__/references/`, `/` 또는 `./` 시작 → 경로로 해석 |
