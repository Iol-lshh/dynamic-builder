...
1. **settings.local.json 확인**: `__AGENT_HOME__/settings.local.json`을 읽고 `env.FIGMA_TOKEN` 키가 있으면 사용
2. **settings.json 확인**: `__AGENT_HOME__/settings.json`을 읽고 `env.FIGMA_TOKEN` 키가 있으면 사용
...
5. **등록 제안**: 토큰을 받으면 재사용을 위해 settings.local.json에 등록할지 묻는다
   ...
   - 승인 시: `__AGENT_HOME__/settings.local.json`의 `env.FIGMA_TOKEN`에 저장
   - 거부 시: 현재 세션에서만 사용
...
- 캐시 경로: `__AGENT_HOME__/figma-cache/<FILE_KEY>/<NODE_ID_SAFE>.json`
