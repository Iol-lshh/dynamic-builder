...
`templates/`의 워크플로우 YAML 정의를 읽어 `__AGENT_HOME__/skills/{name}/`에 실행 가능한 skill을 생성한다.
...
```bash
node ~/.dynamic-builder/.agent/common/scripts/build-workflow.js
node ~/.dynamic-builder/.agent/common/scripts/build-workflow.js --watch
node ~/.dynamic-builder/.agent/common/scripts/build-workflow.js --clean
```
...
- `__AGENT_HOME__/skills/{name}/`의 빌드 결과물을 직접 수정하지 않는다. `src/`나 `templates/`을 수정하고 다시 빌드한다.
