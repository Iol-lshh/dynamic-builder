...
WORKTREE_DIR="$REPO_ROOT/.gemini/worktrees"
...
# worktree 목록 수집 (.gemini/worktrees 하위만)
collect_worktrees() {
  git worktree list --porcelain | awk '
    /^worktree / { path=$2 }
    /^branch /   { branch=$2; if (path ~ /\.gemini\/worktrees\//) print path "\t" branch }
...
collect_prunable() {
  git worktree list --porcelain | awk '
    /^worktree / { path=$2 }
    /^branch /   { branch=$2 }
    /^prunable/  { if (path ~ /\.gemini\/worktrees\//) print path "\t" branch }
  '
}
