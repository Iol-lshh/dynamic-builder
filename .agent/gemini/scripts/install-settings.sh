#!/usr/bin/env bash
# install-settings — hooks와 settings.json을 ~/.gemini에 설치한다
# 사용법:
#   bash .agent/gemini/scripts/install-settings.sh
...
echo "=== Install Settings (Gemini) ==="

# ── hooks 복사 ──────────────────────────────────────────
src_hooks="$DYNAMIC_BUILDER_DIR/.agent/common/hooks"
dst_hooks="$GEMINI_DIR/hooks"
if [[ -d "$src_hooks" ]]; then
  mkdir -p "$dst_hooks"
...
# ── settings.json 머지 ──────────────────────────────────
src_settings="$DYNAMIC_BUILDER_DIR/.agent/gemini/settings.json"
dst_settings="$GEMINI_DIR/settings.json"
